...
type SCiphertext struct {
	*SElement
}

func ConvertTosCiphertext(Ciphertext *Ciphertext) (sciphertext *SCiphertext) {
	converetd := &SCiphertext{ConvertToSelement(*Ciphertext.Element)}
	return converetd
}

func ConvertToCiphertext(sciphertext *SCiphertext) (ciphertext *Ciphertext) {
	//ele := ConvertToElement(*sciphertext.SElement)
	converted := &Ciphertext{ConvertToElement(*sciphertext.SElement)}
	return converted
}
