...
func ConvertToSelement(element Element) *SElement {
	return &SElement{Value: element.value, Scale: element.scale, IsNTT: element.isNTT}
}
func ConvertToElement(element SElement) *Element {
	return &Element{value: element.Value, scale: element.Scale, isNTT: element.IsNTT}
}
