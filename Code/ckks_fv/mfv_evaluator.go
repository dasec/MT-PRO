...
func (eval *mfvEvaluator) ModSwitch(ct0 *Ciphertext, ctOut *Ciphertext) {
	...
	if ct0.Degree() != ctOut.Degree() {
		//fmt.Printf("ct0 degree %v ctOut degree %v: ", ct0.Degree(), ctOut.Degree())
...
func (eval *mfvEvaluator) ModSwitchMany(ct0, ctOut *Ciphertext, nbModSwitch int) {
	...
	//fmt.Printf("MODSWITCHMANY: ct0 lvl %v ctOut lvl %v: nbModSwitch: %v \n", ct0.Level(), ctOut.Level(), nbModSwitch)
	level := ct0.Level()
...
func (eval *mfvEvaluator) SlotsToCoeffs(ct *Ciphertext, stcModDown []int) (ctOut *Ciphertext) {
	...
	for i := 0; i < depth-1; i++ {
		//CZ: added length check to avoid breakdown of FW
		if len(stcModDown) > i && stcModDown[i] > 0 {
	...
	//CZ: added length check to avoid breakdown of FW
	if len(stcModDown) > depth && stcModDown[depth-1] > 0 {
